...
{PRODUCT} 支持很多不同的 assets，你可以用来定义不同的商业决策。 每一个决策过程 asset 有不同的优点，你可以根据自己的目标或者需求来选择一个或者组合多个。
...
a|
* Are tables of rules that you create in a UI-based table designer in {CENTRAL}， 可以在 {CENTRAL} 中通过界面设计来创建的规则表
* Are a wizard-led alternative to spreadsheet decision tables，相较于 spreadsheet decision tables, 多增加向导程序
