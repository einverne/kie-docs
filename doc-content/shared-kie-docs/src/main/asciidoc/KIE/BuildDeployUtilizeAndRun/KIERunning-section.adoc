...
`KieRuntime` 提供规则和流程可使用的方法，比如设置全局变量或者注册channels。
...
Globals 是有名字的对象，能够被 {DECISION_ENGINE} 可见，但是有一个很基础的不同就是，对象中的修改被提交到 global 中时不会触发规则的重新求值。
并且，globals 可以有效地提供静态信息，比如在 RHS 规则中会使用的服务，或者作为从 {DECISION_ENGINE} 中返回对象的方法。
当在 LHS 规则中使用 global 时，确保它是不可修改的，或者至少，不要期望改变会影响规则的行为。

A global must be declared in a rules file, and then it needs to be backed up with a Java object.

global 必须定义在规则文件中，然后需要在 Java Object 中设置。
...
当 KIE base 感知到 global 表示和其类型，他会调用 `ksession.setGlobal()` 来关键 global。
在 DRL 代码中定义 global 类型失败时会导致抛出异常。
...
`RuleRuntimeEventManager` 允许添加或者移除 listeners，这样 working memory 和 agenda 的事件就能够被监听。
...
下面的代码片段显示了一段简单的附加到 Session 上的 agenda 监听器，它将会在规则 fired 时输出匹配的规则。
...
Drools 也提供了 `DebugRuleRuntimeEventListener` 和 `DebugAgendaEventListener`，这两个 listener 提供了打印 debug 日志的方法。
可以通过如下方法来添加 listener。
...
Stateless 是一次性可执行的 KieRuntime. 它可以获取一些结果，然后在结束时销毁，禁止更多的交互。
