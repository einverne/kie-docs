...
这一节描述了管理员通过直观友好的界面如何管理应用的用户，组织，权限，控制谁可以访问不同的资源和功能
...
{CENTRAL} 安全方面定义了三个种类: __user__, __group__ and __role__.
...
The security entities are being registered in the domain by consuming some __realm__. realm 可以是应用程序自己的，比如(Wildfly, EAP, Tomcat) 或者外部支持的, 比如, 使用 Keycloak 服务器处理目标 realm .
...
权限通常是用户可以在应用中操作的事情。通常是一个和具体资源相关的动作，比如：
...
On the other hand, when either using a custom security provider or using one of the available, consider the following installation options:
...
在现有的基础上升级到 7 版本，唯一授予组织，Repositories 或者项目权限资源的方法就是确定哪一些角色可以访问给定的实例。这些角色作为实例持久化状态存储在 GIT 中。 CLI 工具可以用来增加或者删除角色：
