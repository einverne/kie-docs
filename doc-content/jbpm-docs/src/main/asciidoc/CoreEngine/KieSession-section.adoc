...
session 提供了方法来注册或者移除 listeners
...
ProcessEventListener 可以用来监听流程相关的时间，比如开始或者结束一个流程，进入或者离开一个节点，等等。
...
下面是 ProcessEventListener 的不同方法。
...
关于 before 和 after 事件的注意事项：这些事件表现为 stack （栈），这也就意味着 listener 的事件会前后发生，而真正的事件会在 listener 回调中间发生。
...
比如，如果因为离开节点而触发后续节点，node 触发事件会发生在 beforeNodeLeftEvent 和 afterNodeLeftEvent 之间。这样做就让我们能够追溯这两个事件发生的关系。
...
同样的，所有节点触发和节点离开事件会在 beforeProcessStarted 和 afterProcessStarted 之间发生。
...
通常来说，如果你仅仅想要在特定事件发生时被通知，你应该只关心 before 事件（通常他们会在事件真正发生之前被调用）。当只关心 after 事件时，可能会造成事件触发的顺序是错误的印象，但是因为 after 事件是作为 stack 来触发（after 事件只会在对应的 before 事件被触发后被触发）。after 事件只有当你想要确保所有相关过程结束时才推荐使用（比如，当你想要在特定过程结束时被通知）
...
并且，注意并不是所有节点会产生 node triggered 或者 node left 事件，根据不同 node 的类型，一些 nodes 可能只会产生 node left 事件，而另一些可能只会产生 node triggered 事件。
...
jBPM 开箱即用提供了一个 listener ，可以被用来创建审计 log (既可以打印到 console，也可以是文件系统上的文件）。这个审计日志包括了所有在运行时发生的事件，可以轻松地看到发生了什么。
